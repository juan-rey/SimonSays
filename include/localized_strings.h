...
- `F1`: Open Help.
- `F2`: Open Settings.
- `F3`: Open the Gaze / Dwell-click window.
...
## Settings (F2)
Open Settings (`F2`)HELP" LR"HELP() to control:
...
- `F3`: Abrir la ventana de Mirada / Clic por fijación.
...
- **Texto predeterminado**: activar/desacti)HELP" LR"HELP(var `Insertar texto predeterminado automáticamente` y personalizar frase por defecto.
...
## الإعدادات (F2)
افتح الإعدادات (`F2`) للتحكم )HELP" LR"HELP(في:
...
- **Testu lehenetsia**: gaitu/desgaitu `Txertatu testu lehenetsia automatikoki` eta pertsonalizatu esaldi lehenetsia.
  - `U)HELP" LR"HELP(kipen teklatua erakutsi` — pantailako ukipen teklatua irekitzen du sarrera-koadroak fokua jasotzen duenean, leiho nagusitik gertu kokatuta.
...
- **Text predeterminat**: activa/desactiva `Insereix el text predeterminat automàti)HELP" LR"HELP(cament` i personalitza la frase per defecte.
...
- **Texte par défaut** : activer/désactiver `Insére)HELP" LR"HELP(r automatiquement le texte par défaut` et personnaliser la phrase par défaut.
...
- **Texto predeterminado**: activar/desactivar `Inserir automaticamente o texto predeterminado` e personalizar a frase por defecto.
  - `Mostrar teclado táctil` — abre o teclado táctil en pantalla cando )HELP" LR"HELP(a caixa de entrada recibe o foco, posicionado preto da xanela principal.
...
- **Standardtext**: `Standardtext automatisch einfügen` aktivieren/deaktivieren und Standardphrase anpassen.
  - `Bildschirmtastatur anzeigen` — öffnet)HELP" LR"HELP( die Bildschirmtastatur, wenn das Eingabefeld den Fokus erhält, in der Nähe des Hauptfensters positioniert.
...
## הגדרות (F2)
פתח הגדרות (`F2`) כדי )HELP" LR"HELP(לשלוט ב:
...
- **डिफ़ॉल्ट पाठ**: `डिफ़ॉल्ट पाठ स्वतः जोड़ें` )HELP" LR"HELP(सक्षम/अक्षम करें और डिफ़ॉल्ट वाक्य अनुकूलित करें।
...
- **Testo predefinito**: attiva/disattiva `Inserisci automaticamente il testo predefinito` e personalizza la frase predefinita.
  - `Mostra la tastiera)HELP" LR"HELP( touch` — apre la tastiera touch su schermo quando la casella di input riceve il focus, posizionata vicino alla finestra principale.
...
## 設定（F2）
設定（`F2`）を開いて制御：
- **既定のテキスト)HELP" LR"HELP(**: `既定のテキストを自動挿入` を有効/無効にし、既定のフレーズをカスタマイズ。
...
- **Texto predefinido**: ativar/desativar `Inserir texto predefinido automaticamente` e personalizar a frase predefinida)HELP" LR"HELP(.
...
- **Текст по умолчанию**: включить/выключить `Автомат)HELP" LR"HELP(ически вставлять текст по умолчанию` и настроить фразу по умолчанию.
...
## Configuració (F2)
Obre Configuració (`F2`) per controlar:
- **Text predeterminat**: activa/desactiva `Insereix el text predeterminat automàti)HELP" LR"HELP(cament` i personalitza la frase predeterminada.
