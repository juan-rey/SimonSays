...
| `CAT-` | Categories & Phrases management | [`docs/specs/categories-phrases.spec.md`](specs/categories-phrases.spec.md) | **Active** | Category/phrase browse + CRUD; `##` icon prefix (emoji/`.ico`/`.png`/`.jpg`); `::` inline-audio suffix; immediate-speak option; ordering criteria for the built-in default sets. |
